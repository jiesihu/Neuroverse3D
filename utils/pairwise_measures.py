...
from .utils_MetricReloaded import (
    one_hot_encode,
...
class BinaryPairwiseMeasures(object):
    ...
    def calculate_l1(self,prediction,reference):
        """
        Calculate L1 loss between prediction and reference.
        """
        return np.mean(np.abs(prediction - reference))

    def calculate_mse(self,prediction,reference):
        """
        Calculate Mean Squared Error (MSE) between prediction and reference.
        """
        return mean_squared_error(reference, prediction)

    def calculate_psnr(self,prediction,reference):
        """
        Calculate Peak Signal-to-Noise Ratio (PSNR) between prediction and reference.
        """
        data_range = reference.max() - reference.min()
        return peak_signal_noise_ratio(reference, prediction, data_range=data_range)
    def get_ssim(self,output,target):
        '''
        Ref: https://github.com/jorge-pessoa/pytorch-msssim
        '''
        output = (output - output.min()) / (output.max() - output.min())
        target = (target - target.min()) / (target.max() - target.min())
        ss = ssim(output, target, data_range = 1.0, size_average = False, win_sigma = 1.5)
        return ss.mean().cpu().numpy()

    def get_ms_ssim(self,output,target):
        '''
        Ref: https://github.com/jorge-pessoa/pytorch-msssim
        In our cases, Image too small for Multi-scale SSIM computation.
        '''
        output = (output - output.min()) / (output.max() - output.min())
        target = (target - target.min()) / (target.max() - target.min())
        try:
            ms_ss = ms_ssim(output, target, data_range = 1.0, size_average = False, win_sigma = 1.5)
            return ms_ss.mean().cpu().numpy()
        except:
